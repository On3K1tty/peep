...
const DEAD_ZONE_MOTION_DEG = 5.5; /* больше мёртвая зона для motion — меньше дрейф */
...
export class GyroCamera {
  ...
  /** Управление по акселерометру (DeviceMotion) — только если нет ориентации (приоритет у orientation). */
  private _deadZoneMotion(v: number): number {
    if (Math.abs(v) < DEAD_ZONE_MOTION_DEG) return 0;
    return v > 0 ? v - DEAD_ZONE_MOTION_DEG : v + DEAD_ZONE_MOTION_DEG;
  }
  ...
  private _onMotion(e: DeviceMotionEvent) {
    ...
    const dPitch = this._deadZoneMotion(beta - this._baseBeta);
    const dGamma = this._deadZoneMotion(gamma - this._baseGamma);
